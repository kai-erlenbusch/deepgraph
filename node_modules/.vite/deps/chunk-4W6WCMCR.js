...
var Animation = class {
  ...
  start() {
    const update = (time2, frame) => {
      ...
      if (this._animationLoop !== null) this._animationLoop(time2, frame);
    };
    update();
  }
  stop() {
...
var attributeArray = (count, type = "float") => {
  ...
  const buffer2 = new StorageBufferAttribute(count, itemSize);
  const node = storage(buffer2, type, count);
  return node;
};
var instancedArray = (count, type = "float") => {
  const itemSize = getLengthFromType(type);
  const buffer2 = new StorageInstancedBufferAttribute(count, itemSize);
  const node = storage(buffer2, type, count);
  return node;
};
...
var PMREMGenerator = class {
  ...
  fromScene(scene, sigma = 0, near = 0.1, far = 100, renderTarget = null) {
    ...
    if (this._hasInitialized === false) {
      ...
      this.fromSceneAsync(scene, sigma, near, far, cubeUVRenderTarget2);
...
var WebGPUAttributeUtils = class {
  ...
  updateAttribute(attribute2) {
    ...
    if (updateRanges.length === 0) {
      ...
      for (let i = 0, l = updateRanges.length; i < l; i++) {
        const range2 = updateRanges[i];
        const dataOffset = range2.start * byteOffsetFactor;
        const size = range2.count * byteOffsetFactor;
        device.queue.writeBuffer(
          buffer2,
          0,
          array,
...
var ClippingGroup = class extends Group {
  constructor() {
    super();
...
three/build/three.webgpu.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-4W6WCMCR.js.map
